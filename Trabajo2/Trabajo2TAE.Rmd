...
Creamos los dos conjuntos de puntos y los graficamos para ver si son no lineales.
...
Creamos los datasets de entrenamiento y de prueba.
...
Entrenamos la maquina de soporte vectorial con un kernel radial.
...
El entrenamiento con un kernel radial tiene menos errores que el entrenamiento con kernel lineal por lo tanto tiene merjor rendimiento.

# Punto 5

El objetivo de este punto es aprender cómo obtener límites de decisión para para clasificación a partir de una regresión logística aplicando transformaciones no lineales

## Literal a), Creacion del dataset

Creamos los puntos (como nos dice el libro)
...
Graficamos el dataset con diferentes colores para cada clase
...
Ajustamos una regresión logística al dataset
...
Aplicamos el modelo y graficamos
...
Como se esperaba el límite de decisión es linear y no se ajusta correctamente a los datos

## Literal e), Regresión logística con transformaciones no lineales

Ahora ajustamos una regresión logística, pero usando funciones no lineares como “predictors”
...
Aplicamos el nuevo modelo y graficamos
...
Ahora el límite de decisión es no linear y se ajusta mucho más a los datos 

## Literal g), Ajuste de un clasificador vectorial

Para este caso utilizaremos una máquina de soporte vectorial con kernel linear y graficamos los resultados
...
De igual forma que con la regresión logística el límite de decisión es linear y no se ajusta correctamente a los datos

## Literal h), Máquina de soporte vectorial con kernel no-linear

En este caso utilizaremos un kernel radial
...
En este punto se nos muestra que si usamos una máquina de soporte vectorial con kernel no-linear o una regresión logística con transformaciones puede ser muy útil para encontrar límites de decisión no lineales, en cambio si se usa un kernel linear o no se usan transformaciones los resultados están muy alejados de la realidad para este tipo de problemas